...
const Globe = () => {
  ...
  useEffect(() => {
    ...
    createButton('−', () => gsap.to(camera.position, { z: camera.position.z + 0.5, duration: 1, ease: 'power2.out' }));
    createButton('H', () => gsap.to(camera.position, { x:0, y:0, z: 2, duration: 1, ease: 'power2.out' }));

     // Hint text
    const hint = document.createElement('div');
    hint.className = 'absolute bottom-5 left-5 text-white/50 text-xs';
    hint.innerHTML = '🌍 Drag to rotate • Hover pins • Click pins to zoom';
    ...
    const animate = () => {
      ...
      pinObjects.children.forEach(pin => {
          ...
          if (pin.userData.label) {
             ...
             pin.userData.label.style.transform = `translate(${screenX}px, ${screenY}px)`;
          }
      });
    ...
    return () => {
      ...
      if (mountRef.current && uiContainer.parentNode === mountRef.current) {
         mountRef.current.removeChild(uiContainer);
      }
       if (mountRef.current && hint.parentNode === mountRef.current) {
        mountRef.current.removeChild(hint);
      }
       pinObjects.children.forEach(pin => {
          if (pin.userData.label && pin.userData.label.parentNode === mountRef.current) {
             mountRef.current?.removeChild(pin.userData.label);
          }
      });
    };
  }, []);
